...
**Moxfield debug screenshots:** Set `ETL_DEBUG_SCREENSHOTS` to a truthy value (`1`, `true`, `yes`, `on`) to capture Playwright ONGs during the Moxfield (CapSolver) flow, save them under `.data/`, and upload each run’s set to `s3://<bucket>/etl/screenshots/<UTC_timestamp>/` (same timestamp format as the CSV keys). Default is off. On Fargate, Terraform variable `etl_debug_screenshots` maps to this env var (`false` → `ETL_DEBUG_SCREENSHOTS=false`). S3 upload failures are logged and do not fail the sync. Independent of `LOG_LEVEL`.
